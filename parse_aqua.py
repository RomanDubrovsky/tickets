...
with open('222.txt', 'r', encoding='utf-8', errors='ignore') as f:
    text = f.read()
...
for line in lines:
    if line.startswith('https://aquasound.club/') or line.startswith('Это афиша на главной https://aquasound.club/'):
        if current_url:
...
for u, body in pages_raw:
    ...
    slug_m = re.search(r'aquasound\.club/([^/\s\?#]*)', clean_u)
...
print(f'Total extracted events for aquasound: {len(events_aqua)}')
for e in events_aqua:
    print('Slug: ' + e['slug'] + ' | Hash: ' + e['access_hash'] + ' | Title: ' + e['title'])

with open('extracted_events_aqua.json', 'w', encoding='utf-8') as out:
    json.dump(events_aqua, out, ensure_ascii=False, indent=2)
